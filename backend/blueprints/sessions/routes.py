            ...
            result = await db_session.execute(
                ...
                .where(
                    and_(
                        ...
                        == True,  # Only show sessions that haven't been logged out
                    )
                )
                .order_by(
            ...
            return jsonify(
                {
                    ...
                    "current_session_id": session.get(
                        "session_id"
                    ),  # Highlight which one is "this device"
                }
            )
            ...
            result = await db_session.execute(
                select(UserSession).where(
                    and_(
                        ...
                        UserSession.user_id
                        == session["user_id"],  # Security: only your own sessions
                    )
                )
            )
            ...
            result = await db_session.execute(
                select(UserSession).where(
                    and_(
                        ...
                        != current_session_id,  # Don't log out ourselves
